...
$.jQchart={
	version  : '0.03-beta-1.0',
	updateMemo : 'v0.03-beta-1.0 are going to support CanvasGradient.',
...
$.fn.jQchart= function(op){ 
	...
	if($.browser.msie && !$.fn.jQchart.winloaded){
	...
	function drawChart(_option){
		...
		jQuery.extend( $.fn.jQchart,{
			...
			wrtTitle :	function(op){
				op.subclass = 'title' ;
				op.subid = "#jQchart-title-T-"+op.id;
			...
			wrtXscale	: function(op){
				...
				for (var i = 0, currP=0; i < op.grid.col;i++) {
					op.subid = 'jQchart-labelX_'+i ;
					
					this.labelX_wrtText(x+ 'px',y+ 'px',(op.xGap-2)+ 'px',
						op.labelX[currP]+" ",
						op,
			...
			wrtYscale	: function(op){
				...
				var x=op.scaleYLeft ,
					y=op.chartHeight - op.scaleFontSize ;
						
				for (var i = 0, currP=0; i <= op.grid.row;i++) {
					
					op.subid = 'jQchart-labelY_'+i;
			...
			wrtText : function(x,y,text,op,scope){
				...
				if(op.subid) var subid=op.subid;
				
				return $('<div id="'+subid+'" class="jQchart-'+subclass+'-'+op.id+' jQchart-'+subclass+'"></div>')
					.append(text)
					.css({
			...
			labelX_wrtText : function(x,y,width,text,op,scope){
